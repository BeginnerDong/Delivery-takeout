			...
			<div class="myaddress-lis boxShaow radius10" v-for="(item,index) in mainData">
				...
				<div class="seltBox">
					<div class="L" :data-id="item.id" @click="updateCard($event.currentTarget.dataset.id)">
						<img class="icon" :src="item.isdefault==1?'../../static/images/address-icon.png':'../../static/images/address-icon2.png'">
						默认银行卡
					</div>
					<div class="R fs12">
						<a class="child" :data-id="item.id" @click="Router.navigateTo({route:{path:'/pages/user_myBankCard_add/user_myBankCard_add?id='+$event.currentTarget.dataset.id}})">
							<img  src="../../static/images/address-icon1.png" mode=""></image>编辑
						</a>
						<div class="child" :data-id="item.id" @click="deleteCard($event.currentTarget.dataset.id)">
							<img  src="../../static/images/management-icon1.png" mode=""></image>删除
						</div>
					</div>
				</div>
			</div>

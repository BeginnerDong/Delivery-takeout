			...
			<div class="myaddress-lis boxShaow radius10" v-for="(item,index) in myBankData">
				...
				<div class="seltBox">
					<div class="L" @click="change(index)">
						<image class="icon" :src="curr==index?'../../static/images/address-icon.png':'../../static/images/address-icon2.png'"/>
						默认银行卡
					</div>
					<div class="R fs12">
						<div class="child" @click="Router.navigateTo({route:{path:'/pages/user_myBankCard_edit/user_myBankCard_edit'}})"><img src="../../static/images/address-icon1.png" mode=""/>编辑</div>
						<div class="child" @click="deltAlert"><img src="../../static/images/management-icon1.png" mode=""/>删除</div>
					</div>
				</div>
			</div>
			
			<div class="submitbtn" style="margin-top: 80px;">
				<button type="button" class="btn" @click="Router.navigateTo({route:{path:'/pages/user_myBankCard_add/user_myBankCard_add'}})">添加银行卡</button>
			</div>
...
<style>
	...
	.btnB div{width: 50px; line-height: 25px; height: 25px;border-radius: 15px;border:1px solid #999;}

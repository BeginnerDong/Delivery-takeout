	...
	<div>
		...
		<div class="inde_prolis pr" style="min-height: 260px;">
			<scroll-view class="store_LeftNav center" scroll-y="true" :style="'height:'+(windowHeight*2 - 380)+'rpx'">
				<div class="item" v-for="(item,index) in labelData" :class="curr==index?'on':''" :key="index" @click="changeNav(index)">
					{{item.title}}
				</div>
			</scroll-view>
				...
				<div class="shopOrder" v-for="(item,index) in mainData" :id="item.id">
					...
					<div class="item">
						<div class="infor" style="margin: 30rpx 0;" v-for="(c_item,c_index) in item.data" :key="c_index">
							...
							<div class="rr">
								<div class="titile fs13 avoidOverflow" :data-id="c_item.id" @click="Router.navigateTo({route:{path:'/pages/goodsDetail/goodsDetail?id='+$event.currentTarget.dataset.id}})">
									{{c_item.title}}
								</div>
								<div class=" Bmny">
	...
	export default {
		...
		methods: {
			...
			counter(index, c_index, type) {
				...
				if (type == '+') {
					if (self.mainData[index].data[c_index].stock <= self.mainData[index].data[c_index].count) {
						uni.showModal({
							title: '提示',
							content: '该商品库存不足！',
							confirmColor: '#ca1c1d',
			...
			counterTwo(index, type) {
				...
				if (type == '+') {
					...
					if (childItem[index].stock <= childItem[index].count) {
						uni.showModal({
							title: '提示',
							content: '该商品库存不足！',
							confirmColor: '#ca1c1d',
